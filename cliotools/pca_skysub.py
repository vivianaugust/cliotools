...
def badpixfix(image,badpixels,xca,yca,xcb,ycb, boxsize = 300, dx = 5):
    ''' Fix all the bad pixles in the provided bad pixels map that are within the specified
    ...
    Args:
        ...
        boxsize (int): only fix pixels within a box of size boxsize x boxsize from the star
        dx (int): how many pixels to the side should be used for interpolation. Should be odd.
    '''
    imfix = image.copy()
    boxa = np.array([int(yca-0.5*boxsize),int(yca+0.5*boxsize),
                     int(xca-0.5*boxsize),int(xca+0.5*boxsize)])
    boxb = np.array([int(ycb-0.5*boxsize),int(ycb+0.5*boxsize),
                     int(xcb-0.5*boxsize),int(xcb+0.5*boxsize)])
    badpix_list = np.where(~np.isnan(badpixels))
    for j in range(len(badpix_list[0])):
        x = badpix_list[1][j]
        y = badpix_list[0][j]
        if y >= boxa[0] and y <= boxa[1] and x >= boxa[2] and x <= boxa[3]:
            imfix[y,x] = fix_pixel(image, (x,y))
        elif y >= boxb[0] and y <= boxb[1] and x >= boxb[2] and x <= boxb[3]:
            imfix[y,x] = fix_pixel(image, (x,y))
        else:
            pass
    return imfix
